<template>
	<div class="wrap">
		...
		<div class="index_nav">
			...
			<div class="dingzhi">
				...
				<div class="dingzhi_info_box">
					...
					<input v-model="phone" class="telphone" maxlength="11" placeholder="联系方式" type="text" />
					<textarea v-model="demand" class="content" placeholder="请填写您的需求，限一百字">
		...
		<div class=" mudi_box">
			<div class="mudi_row">
				<div class="mudi_item" v-if="index<3" v-for="(item,index) in townInfo" @click="Totown(item)">
					<div class="thumb" :style="{backgroundImage: 'url(' + item.town_thumb + ')'}"></div>
		...
		<div class="index_town play_box">
			...
			<div @click="todetail(activity.activity_id)" class="play_item" v-for="activity in activitys" 
	...
	export default {
		...
		methods: {
			...
			dingzhi: function () {
				...
				var self = this
				// console.log(!self.contact_person,!self.phone,!self.demand)
				if(!self.demand||!self.contact_person||!self.phone){
